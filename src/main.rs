...
fn main() {
    ...
    .subcommand(Command::new("coverage")
        ...
        .subcommand(Command::new("coverage-picture")
            .about("Visualize the coverage depth into a png image. Beware: the height is equal to the number of targets, and the width is equal to the length of the longest target. Use --resolution to reduce the width to avoid having an enormous image. To reduce the height, check out concat-into-rows.")
            .arg_required_else_help(true)
            .arg(Arg::new("infile")
